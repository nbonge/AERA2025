...
mach.dist.small <- mach.dist %>%
  dplyr::select("id", "item", "theta", "tau3", "discrim",
                "abs.dist", "discrim.abs.dist", "endorse", "endorse1")
...
figure.2 <- plot(figure.2.data, show_ci = F, line_size = 0.75) + 
  labs(x = expression(paste("Discrimination-adjusted ability-difficulty distance ",italic("a"), "|θ − τ|", sep = "")), 
...
# 7.0 Post-Hoc Analyses ----
na.rows <- which(is.na(dat_all$testelapse))

# filtering out 3 respondents who did not take the test at all (testelapse time is NA)
dat_full <- dat_all |>
  filter(!(row_number() %in% na.rows))

# filtering responses to US only
dat_low_quality <- dat_full |>
  filter(country == "US")

dat_low_quality$id <- c(1:nrow(dat_low_quality))


# checking for longstrings
long <- longstring(dat_low_quality[paste0("Q", 1:20, "A")], avg = TRUE)
long$id <- c(1:nrow(long))

# "faking"?
dat_no_fakes1 <- dat_low_quality |>
  mutate(item1 = Q1A,
         item2 = Q2A,
         item3R = 6 - Q3A,
         item4R = 6 - Q4A,
         ...
         item19 = Q19A,
         item20 = Q20A)
fakes_longstrings <- longstring(dat_no_fakes1[,110:129], avg = TRUE)
describe(fakes_longstrings$longstr, quant = c(.01, .05, .10, .25, .50, .75, .90, .95, .99))
hist(fakes_longstrings$longstr, breaks = 20)


data_fakes <- fakes_longstrings |>
  mutate("id" = c(1:nrow(fakes_longstrings))) |>
  mutate("fakeFlag" = case_when(
    longstr > 8 ~ 1,
    .default = 0)) |>
  select("id", "fakeFlag")

# getting all flagged low quality responses 
dat_low_quality_flag <- left_join(x = dat_low_quality, 
                                  y = long,
                                  by = "id") |>
  left_join(data_fakes, by = "id")


dat_low_quality_flag <- dat_low_quality_flag |>
  mutate("test.ntile" = ntile(dat_low_quality_flag$testelapse, 10),
         "survey.ntile" = ntile(dat_low_quality_flag$surveyelapse, 10)) |>
  mutate("flag.case" = case_when(
    dat_low_quality_flag$age >= 100 ~ 1,
    dat_low_quality_flag$test.ntile == 10 ~ 2,
    dat_low_quality_flag$test.ntile == 1 ~ 3,
    dat_low_quality_flag$survey.ntile == 10 ~ 4,
    dat_low_quality_flag$survey.ntile == 1 ~ 5,
    VCL6 == 1 ~ 6,
    VCL9 == 1 ~ 7,
    VCL12 == 1 ~ 8,
    longstr > 6 ~ 9,
  ...
  mutate(item1 = Q1A,
         ...
         item19 = Q19A,
         item20 = Q20A) |>
  dplyr::select(!c("item3R", "item19", "item11R", "item17R", "item16R", "item20"))

# estimating theta values for low quality respondents
all.quality.thetas <- fscores(mod.grm, 
                              response.pattern = dat_low_quality_flag[,115:128], 
                              full.scores.SE = TRUE) |>
  cbind(dat_low_quality_flag[c("id", "flag.case", "flag")]) 
colnames(all.quality.thetas) <- c("theta.est", "theta.est.SE", "id", "flag.case", "flag")

all.quality.thetas <- all.quality.thetas |>
  mutate("flagType" = as.factor(case_when(
    flag.case == 10 ~ "Faking",
    flag.case == 1 ~ "Age",
    flag.case == 2 ~ "TestShort",
    flag.case == 3 ~ "TestLong",
    flag.case == 4 ~ "SurveyShort",
    flag.case == 5 ~ "SurveyLong",
    flag.case >= 6 & flag.case <= 8 ~ "VocabCheck",
    flag.case == 9 ~ "Longstring",
    flag.case == 0 ~ "NoFlag"
  )))

all.quality.thetas |> 
  ggplot(aes(x = theta.est, group = flagType, color = flagType)) + 
  geom_histogram()

all.quality.thetas |> 
  ggplot(aes(x = theta.est)) + 
  geom_histogram() +
  facet_wrap(~flagType)
...
all.thetas.by.flag <- all.quality.thetas |>
  group_by(flag) |>
  mutate("ability.ntile" = ntile(all.quality.thetas$theta.est, 10))
